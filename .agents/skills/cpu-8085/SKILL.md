...
8. **C90 shapes even without C.** When writing 8085 assembly directly, use the kernels in **`compiler-ac85`** (pointer walk vs `a[i]`, word cursor DE, dual cursors, DSUB signed/unsigned, stack-only automatics). Those shapes produce better 8085 than a naïve statement-by-statement lowering.
...
- Function header (purpose, inputs, outputs, uses): **`compiler-ac85`**, including hand-written routines.
...
Document every slot. Function header: **`compiler-ac85`**. Drop consumed args in one epilogue:
...
Do not copy Z80 `dec bc; jp nz` semantics. **C `n--` / `--i == 0` is not `jp k` / `jp nk`** (K is −1, and 16-bit `dec` does not write Z). Those C loops: test Z (`ld a,h` / `or l`) or the `dec bc` / `inc b` / `inc c` structure below. Full C90 loop shapes: **`compiler-ac85`**.
...
- **New 8085-only / agent listings:** blank line after `jp` / `jp cc` / `jp (hl)` / `ret` / `ret cc`. **Not** after `call` / `call cc`. Write `jp`, not `jr` (`compiler-ac85` Listing layout).
...
**`jr` / `jr cc`:** allowed in **normal** mode as `jp` / `jp cc`. Use them when the same source may also build for Z80. 8085-only / agent listings write `jp` (`compiler-ac85`). **Strict** / `-no-synth` rejects `jr` and all saccharine. Cost `jr` as a 3-byte `jp` (cond 10/7). `18` is native `rl de`, not `jr`.
...
9. Name the kernel from **`compiler-ac85`** (even for hand-written asm) before emitting opcodes.
...
- C90 shapes for better 8085 asm (hand-written or from C): `compiler-ac85`
