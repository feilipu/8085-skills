...
**Not on 8085** (Z80 CB / illegal pair forms — do not emit; `compiler-ac85` Do not emit): `srl r` / `srl hl`, `bit n,r`, `ld bc,sp+*`, `ld bc,(de)` / `ld (de),bc`. `CB` is `rst v`. LDSI is `ld de,sp+*` only.
