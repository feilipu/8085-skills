...
| `malloc` / `free` / `memcpy` / `strlen` / `strcpy` / `strcmp` / `qsort` | hosted libc | Bind the preprocessed `PUBLIC`. Do not open-code unless the count is tiny and in the TIMER region |
